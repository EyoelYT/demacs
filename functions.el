...
(defun ey/backward-to-bol-or-indent (&optional point)
  "Like `doom/backward-to-bol-or-indent' but integrates org mode bol
behavior as well"
  (interactive "^d")
...
(defun ey/+org-capture-project-todo-file ()
  "Find the nearest todo.org in a parent directory, otherwise,
opens a blank one at the project root. Throws an error if not in a project."
  (+org--capture-local-root "todo.org"))

;;;###autoload
(defun ey/+org-capture-project-notes-file ()
  "Find the nearest notes.org in a parent directory, otherwise,
opens a blank one at the project root. Throws an error if not in a project."
  (+org--capture-local-root "notes.org"))

;;;###autoload
(defun ey/+org-capture-project-changelog-file ()
  "Find the nearest changelog.org in a parent directory,
